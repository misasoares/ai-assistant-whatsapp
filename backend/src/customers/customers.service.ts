...
import { Customer, Instance } from '@prisma/client';
...
export class CustomersService {
  ...
  async create(data: { name: string; email: string; plan?: string }): Promise<Customer> {
    ...
    const updatedInstances = await Promise.all(
      customer.instances.map(async (instance) => {
        try {
          ...
          const state = evoState?.instance?.state || 'DISCONNECTED';
  ...
  async update(id: string, data: { name?: string; plan?: string }): Promise<Customer> {
